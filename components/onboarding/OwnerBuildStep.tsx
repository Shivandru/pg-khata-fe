...
export type RoomEntry = { roomNumber: string; floor: number; bedCount: number };
export type PricingEntry = { bedCount: number; rentAmount: number };
...
export function OwnerBuildStep(props: {
  ...
  {rooms.map((room, i) => (
    <div
      key={i}
      className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center rounded-lg border bg-muted/30 px-3 py-2"
    >
      ...
      <button
        type="button"
        className="justify-self-end self-center text-muted-foreground hover:text-destructive transition-colors"
        onClick={() => removeRoom(i)}
        disabled={rooms.length === 1}
      >
        <Trash2 className="h-4 w-4" />
        ...
        <Button className="w-full gap-2" disabled={isBuilding} onClick={onSubmit}>
          {isBuilding ? "Creating..." : "Create Property"}
