...
interface Room { roomId: string; roomNumber: string; floor: number; }
interface Bed { bedId: string; label: string; }
  ...
  const { data: allBeds = [] } = useQuery<Bed[]>({
    ...
    queryFn: async () => {
      const results = await Promise.all(
        rooms.map((r) =>
          api<{ bedId: string; roomId: string; label: string; isOccupied: boolean }[]>(
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {guests.length} active tenant{guests.length !== 1 ? "s" : ""} in {property.name}
          </p>
        </div>
      </div>
